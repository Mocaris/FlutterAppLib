...
class AlertActionDialog extends StatelessWidget {
  ...
  const AlertActionDialog({Key? key, this.title, this.message, this.positive, this.negative, this.backgroundColor})
      : super(key: key);
            ...
            child: Card(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    _getTitle(context),
                    ConstrainedBox(constraints: const BoxConstraints(minHeight: 70), child: _getMessage(context)),
  ...
  _buildOption(BuildContext context) {
    var list = <Widget>[];
    if (null != negative) {
      list.add(Expanded(flex: 1, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 5), child: negative)));
    }
    if (null != positive) {
      list.add(Expanded(flex: 1, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 5), child: positive)));
    }
    return Visibility(
        visible: list.isNotEmpty,
        child: Container(
            margin: const EdgeInsets.only(top: 10),
            child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: list)));
  }
