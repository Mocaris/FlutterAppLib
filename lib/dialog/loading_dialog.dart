...
class LoadingDialog extends StatefulWidget {
  ///[value] range [0.0...1.0]
  const LoadingDialog(this.content, {Key? key, this.outsideDismiss = false, this.onBackDismiss = false, this.backgroundColor, this.controller}) : super(key: key);
...
class _LoadingDialogState extends State<LoadingDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    if (done) {
      progressChild = doneWidget ?? const Icon(Icons.check_circle_rounded);
    } else if (null != pValue) {
      if (pValue! < 1.0) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Visibility(
                          ...
                          child: Center(
                            child: Text(content ?? "", style: const TextStyle(fontSize: 12), maxLines: 2, overflow: TextOverflow.ellipsis),
                          ))
                    ],
                  ),
                ),
